...
LP_SPLITTED_NET_FILE = j(LP_DIR, "benchmark-data", "net={network}_d={directed}_frac={frac}_index={index}.npz")
LP_SPLITTED_EDGE_FILE = j(LP_DIR, "benchmark-data", "test_edgelist_net={network}_d={directed}_frac={frac}_index={index}.csv")
...
#LP_EMB_FILE_ALL = expand(LP_EMB_FILE, network=LP_NET_LIST, **split_params, **emb_params)

#
#
#
#NET_LIST_TABLE = j(FIG_DIR, "network-table.tex")



##
## LFR Benchmark WorkFlow
##
#LFR_DIR = config["lfr_dir"]
#
## LFR_NETWORK_DIR = j(LFR_DIR, "lfr-network")
#LFR_RESULT_DIR = j(LFR_DIR, "results")
#LFR_CONFIG_FILE = j(LFR_DIR, "lfr-config.json")
#LFR_NET_DIR = j(LFR_DIR, "networks")
#LFR_EMB_DIR = j(LFR_DIR, "embeddings")
#
#LFR_PARAMS = {
...
#}
#
#LFR_NET_FILE = j(LFR_NET_DIR, "param_id={param_id}_mu={mu}_s={sample_id}_network.npz")
#LFR_NET_FILE_LIST = expand(LFR_NET_FILE, **LFR_PARAMS)
#
#LFR_COMMUNITY_FILE = j(
#    LFR_NET_DIR, "param_id={param_id}_mu={mu}_s={sample_id}_community_table.csv"
#)
#LFR_COMMUNITY_FILE_ALL = expand(LFR_COMMUNITY_FILE, **LFR_PARAMS)
#
#LFR_PARAM_FILE = j(LFR_NET_DIR, "param_id={param_id}_mu={mu}_s={sample_id}_params.json")
#LFR_PARAM_FILE_ALL = expand(LFR_PARAM_FILE, **LFR_PARAMS)
#
#LFR_EMB_FILE = j(
#    LFR_EMB_DIR,
#    "emb_net_param_id={param_id}_mu={mu}_s={sample_id}_model={model_name}_wl={window_length}_dim={dim}.npz",
#)
#LFR_EMB_FILE_ALL = expand(LFR_EMB_FILE, **LFR_PARAMS, **LFR_EMB_PARAMS)
#
#LFR_RESULT_FILE = j(LFR_RESULT_DIR, "results.csv")
...
#
##
## The validation of the block approximation
##
#APP_DIR = j(DATA_DIR, "sbm-trans-prob-approx")
#APP_NET_LIST = [
#    "ca-AstroPh",
#    "ca-HepTh",
#    "dblp_cite",
...
#    # "soc-epinions",
#]
#APP_RES_FILE = j(APP_DIR, "yypred_net={network}.csv",)
#APP_RES_FILE_ALL = expand(APP_RES_FILE, network=APP_NET_LIST)
#APP_RESULT_FIG = j(FIG_DIR, "result-sbm-approximation.pdf")
#
#FIGS = [
#    LP_RESULT_FIG,
#    LFR_RESULT_FIG,
#    LFR_RESULT_ZOOMED_FIG,
#    LP_RESULT_SI_FIG,
#    LFR_RESULT_SI_FIG,
#    NET_LIST_TABLE,
#]
...
rule eval_link_prediction:
    input:
        net_files=expand(LP_SPLITTED_NET_FILE, network = LP_DATA_SRC.keys(), **split_params),
        emb_files=expand(LP_EMB_FILE, **emb_params, network = LP_DATA_SRC.keys(), **split_params),
        edge_files=expand(LP_SPLITTED_EDGE_FILE, network = LP_DATA_SRC.keys(), **split_params),
    output:
...
#    shell:
#        "python3 workflow/generate-lfr-net.py {input} {params.param_id} {params.mu} {output}"
#
#
...
#    input:
#        emb_files=LFR_EMB_FILE_ALL,
#        com_files=LFR_COMMUNITY_FILE_ALL,
#    output:
